...
export const getWalletsByUser = async (req: Request, res: Response) => {
  try {
    const { uid } = req
    const userWallets = await WalletModel.find({ idUser: uid }).sort(
      '-createdAt',
    )
      ...
      if (currentWallet) {
        if (currentWallet.balance > 0) {
          return res.status(400).json({
            ok: false,
            msg:
              'Esta billetara aun tiene saldo, transfieralo a otra billetara',
          })
        }

        const getUserOwnerWallet = await UserModel.findById(
          currentWallet.idUser,
        )

        let walletIdTemp = walletId

        getUserOwnerWallet!.wallets = getUserOwnerWallet!.wallets.filter(
          (wallet) => walletIdTemp != wallet,
        )
...
export const verifyWalletOwner = async (
  idUser: string,
  idWallet: string,
): Promise<boolean> => {
  const verifyWallet = await WalletModel.findOne({ _id: idWallet }).sort(
    '-createdAt',
  )
